...
class TestSlugify:
    ...
    def test_german_capital_umlauts(self):
    ...
    def test_strips_leading_trailing_delimiter(self):
        assert dict_utils.slugify(" Hello ") == "Hello"

    def test_collapses_repeated_delimiters(self):
    ...
    def test_max_length_strips_trailing_delimiter(self):
...
class TestIsUri:
    def test_https_url_is_uri(self):
        assert dict_utils.is_uri(
            "https://identifier.buildingsmart.org/uri/tst/test/1.0/class/X"
        )

    def test_http_url_is_uri(self):
        assert dict_utils.is_uri("http://example.com/something")

    def test_bare_code_is_not_uri(self):
        assert not dict_utils.is_uri("just-a-code")

    def test_empty_string_is_not_uri(self):
        assert not dict_utils.is_uri("")

    def test_path_without_scheme_is_not_uri(self):
...
class TestGetDictionaryPathFromUri:
    ...
    def test_class_uri_extracts_dict_path(self):
...
class TestBuildBsddUrl:
    def test_builds_class_uri(self):
        data = {
            "namespace": ["hw", "som"],
            "version": "0.2.0",
            "resource_type": "class",
    ...
    def test_no_trailing_slash_by_default(self):
        data = {
            "namespace": ["hw", "som"],
            "version": "0.2.0",
            "resource_type": "class",
    ...
    def test_namespace_as_string(self):
        data = {
            "namespace": "hw/som",
            "version": "0.2.0",
            "resource_type": "class",
...
class TestDictUtilsIsIfcReference:
    def test_ifc_uri_detected(self):
        assert dict_utils.is_ifc_reference(
            "https://identifier.buildingsmart.org/uri/buildingsmart/ifc/4.3/class/Wall"
        )

    def test_non_ifc_uri_not_detected(self):
        assert not dict_utils.is_ifc_reference(
            "https://identifier.buildingsmart.org/uri/hw/som/0.2.0/class/Leiter"
        )
