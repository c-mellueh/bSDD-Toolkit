"""Loin tool — owns the in-memory ISO 7817-3 Level of Information Need document.

This tool stores the LOIN data as a fresh `LoinLevelOfInformationNeed` per
project plus a denormalized membership cache that the property-picker UI uses
to render and edit the UC × MS checkbox grid. All mutation goes through the
class methods here so the relevant Qt signals fire exactly once per change.
"""
...
class Loin:
    """LOIN tool — class-only API per project convention."""
    ...
    def reset(cls) -> None:
        ...
        props.purposes.append(
            LoinPurpose(
                guid=uuid4(),
                names=[DtMultiLangText(language="en", text=cls.DEFAULT_PURPOSE_NAME)],
            )
        )
        props.milestones.append(
            LoinInformationDeliveryMilestone(
                guid=uuid4(),
                names=[DtMultiLangText(language="en", text=cls.DEFAULT_MILESTONE_NAME)],
            )
        )
    ...
    @classmethod
    def rename_purpose(
        cls, purpose_guid: UUID, name: str, language: str = "en"
    ) -> None:
    ...
    def remove_milestone(cls, milestone_guid: UUID) -> None:
        props = cls.get_properties()
        props.milestones = [
            m for m in props.milestones if m.guid != milestone_guid
        ]
        for key in list(props.specs.keys()):
    ...
    def set_milestone_date(
        cls, milestone_guid: UUID, date: Optional[datetime]
    ) -> None:
        for m in cls.get_properties().milestones:
            if m.guid == milestone_guid:
                m.date = date
    ...
    @classmethod
    def milestone_display_name(
        cls, milestone: LoinInformationDeliveryMilestone
    ) -> str:
        if milestone.names:
    ...
    def get_or_create_spec(
        cls, purpose_guid: UUID, milestone_guid: UUID
    ) -> LoinSpecification:
        ...
        milestone = next(
            (m for m in props.milestones if m.guid == milestone_guid), None
        )
    ...
    def is_property_included(
        ...
        props = cls.get_properties()
        per_class = props.properties_in_spec.get(
            (purpose_guid, milestone_guid), {}
        )
    ...
    def set_property_included(
        ...
        if included:
            ...
            if spec_per_obj is not None:
                cls._add_property_to_spec_per_obj(
                    spec_per_obj, bsdd_property
                )
        else:
    ...
    def _remove_property_from_spec_per_obj(
        ...
        if not group.has_property_refs:
            alpha.groups_of_properties.group_of_properties = [
                g
                for g in alpha.groups_of_properties.group_of_properties
                if g is not group
            ]
    ...
    def _property_ref(cls, bsdd_property: "BsddClassProperty") -> DtRef:
        ...
        uri = getattr(bsdd_property, "PropertyUri", None) or getattr(
            bsdd_property, "Uri", None
        )
    ...
    def _adopt_loin(cls, loin: LoinLevelOfInformationNeed,bsdd_dictionary:BsddDictionary) -> None:
        ...
        for spec in loin.specifications:
            ...
            for s in spec.specifications_per_object_type:
                ...
                if s.alphanumerical_information and s.alphanumerical_information.groups_of_properties:
                    for g in s.alphanumerical_information.groups_of_properties.group_of_properties:
                        ...
                        for ref in g.has_property_refs:
                            uri = ref.reference_uri or (
                                str(ref.guid) if ref.guid else None
                            )
                            if uri is None:
                                continue
