...
class AiPropertyDescription:
    @classmethod
    def get_properties(cls) -> AiPropertyDescriptionProperties:
    ...
    def add_ai_to_propertyedit(
        ...
        cls._position_ai_button(widget)
        cls._sync_ai_button(widget)
        widget._ai_button.clicked.connect(lambda: trigger.generate_property_definition(widget))
    ...
    @classmethod
    def _sync_ai_button(cls, widget) -> None:
        is_empty = cls.get_text_widget(widget).toPlainText() == ""
        widget._ai_button.setVisible(is_empty)
