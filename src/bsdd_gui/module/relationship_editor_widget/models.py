...
class RelationshipModel(ItemModel):
    ...
    def get_virtual_list(self) -> list[BsddClassRelation | BsddPropertyRelation]:
        if isinstance(self.bsdd_data, BsddClass):
            items = [cr for cr in self.bsdd_data.ClassRelations if cr not in self.virtual_remove]
        else:
            items = [pr for pr in self.bsdd_data.PropertyRelations if pr not in self.virtual_remove]
